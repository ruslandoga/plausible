defmodule Plausible.Ingestion.WriteBuffer do
  ...
  def handle_cast({:insert, row_binary}, state) do
    ...
    if state.buffer_size >= state.max_buffer_size do
      Logger.debug("#{state.name} buffer full, flushing to ClickHouse")
  ...
  def terminate(_reason, %{name: name} = state) do
    Logger.debug("Flushing #{name} buffer before shutdown...")
    do_flush(state)
  end
  ...
  defp do_flush(state) do
    ...
    case buffer do
      ...
      _not_empty ->
        Logger.debug("Flushing #{buffer_size} byte(s) RowBinary from #{name}")
