defmodule Plausible.SiteAdmin do
  ...
  def transfer_data([from_site], params) do
    ...
    if to_site do
      ...
      {:ok, _} =
        Plausible.ClickhouseRepo.query(event_q, [], timeout: 30_000, command: :insert_select)

      session_q = session_transfer_query(from_site.domain, to_site.domain)

      {:ok, _} =
        Plausible.ClickhouseRepo.query(session_q, [], timeout: 30_000, command: :insert_select)
